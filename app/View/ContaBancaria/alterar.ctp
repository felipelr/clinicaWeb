<div class="">
    ...
    <div>
        <form id="ContaBancariaCadastrarForm" class="form-horizontal" method="post" action="<?php echo $this->Html->url(array("controller" => "conta_bancaria", "action" => "cadastrar")); ?>">
            <input type="hidden" id="idcontabancaria" name="data[ContaBancaria][idcontabancaria]" value="<?php echo $ContaBancaria['idcontabancaria']; ?>" />
            <div class="form-group ">
                <label for="agencia" class="control-label col-md-2">Agência:</label>
                <div class="col-md-8">
                    <input autofocus="autofocus" type="number" id="agencia" name="data[ContaBancaria][agencia]" required placeholder="Informe a agência" class="form-control" value="<?php echo $ContaBancaria['agencia']; ?>"/>
                </div>                        
            </div>
            <div class="form-group ">
                <label for="conta" class="control-label col-md-2">Conta:</label>
                <div class="col-md-8">
                    <input id="conta" type="number" name="data[ContaBancaria][conta]" required placeholder="Informe a conta" class="form-control" value="<?php echo $ContaBancaria['conta']; ?>"/>
                </div>                        
            </div>
            <div class="form-group ">
                <label for="titular" class="control-label col-md-2">Titular:</label>
                <div class="col-md-8">
                    <input id="titular" name="data[ContaBancaria][titular]" required placeholder="Informe o nome do titular" class="form-control" value="<?php echo $ContaBancaria['titular']; ?>"/>
                </div>                        
            </div>
            <div class="form-group ">
            ...
            <div class="form-group ">
                ...
                <div class="col-md-8">
                    <input id="documento" name="data[ContaBancaria][documento]" required placeholder="Informe o documento" class="form-control" value="<?php echo $ContaBancaria['documento']; ?>"/>
                </div>                        
            </div>
            <div class="form-group ">
                    ...
                    <select id="id_banco" name="data[ContaBancaria][id_banco]" data-required="true" class="form-control" title="Banco">
                        ...
                        if ($total_banco > 0):
                            for ($i_banco = 0; $i_banco < $total_banco; $i_banco++) :                                
                                ?>
                                <option value="<?php echo $Bancos[$i_banco]["b"]["idbanco"]; ?>" <?php echo $Bancos[$i_banco]["b"]["idbanco"] == $ContaBancaria['id_banco'] ? 'selected' : ''; ?> ><?php echo $Bancos[$i_banco]["b"]["codigo"] . ' - ' . $Bancos[$i_banco]["b"]["descricao"]; ?></option>
                                <?php
                            endfor;
                        endif;
                        ?>
                    ...
                    <select id="id_paciente" name="data[ContaBancaria][id_paciente]" class="form-control" title="Paciente">
                        ...
                        if ($total_ > 0):
                            for ($i_ = 0; $i_ < $total_; $i_++) :
                                ?>
                                <option value="<?php echo $Pacientes[$i_]["p"]["idpaciente"]; ?>" <?php echo $Pacientes[$i_]["p"]["idpaciente"] == $ContaBancaria['id_paciente'] ? 'selected' : ''; ?> ><?php echo $Pacientes[$i_]["p"]["nome"] . ' ' . $Pacientes[$i_]["p"]["sobrenome"]; ?></option>
                                <?php
                            endfor;
                        endif;
                        ?>
