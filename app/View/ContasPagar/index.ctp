<div class="">
    ...
    <div class="well">
        <form id="form-consultar" class="form-inline" data-action="<?php echo $this->Html->url(array("controller" => "contas_pagar", "action" => "ajax_relatorio")); ?>" method="post">
            ...
            <div class="row-divisao">
                <div class="form-group">
                    <label class="control-label" style="width: 90px;">Favorecido</label>
                </div>
                <div class="form-group" style="min-width: 300px; margin-right: 15px;">                    
                    <select id="favorecidos" class="form-control" name="favorecido">
                        <option value="0">Todos</option>
                        <?php
                        ...
                        if ($total_f > 0):
                            for ($i_f = 0; $i_f < $total_f; $i_f++) :
                                ?>
                                <option value="<?php echo $Favorecidos[$i_f]["f"]["idfavorecido"]; ?>" <?php echo $Favorecidos[$i_f]["f"]["idfavorecido"] == $favorecido_ ? "selected" : ""; ?> ><?php echo $Favorecidos[$i_f]["f"]["nome"]; ?></option>
                                <?php
                            endfor;
                        endif;
                        ?>
...
<!--                        <option value="0">Todos</option>-->
            ...
            <div class="row-divisao">                
                <div class="form-group">
                    <label class="control-label" style="width: 120px;">Nome da despesa:</label>                
                </div>
                <div class="form-group" style="margin-right: 15px;">
                    <input name="nome_despesa" class="form-control" value="<?php echo $nome_despesa_; ?>"/>                 
                </div>                
                <div class="form-group">
                    <label class="control-label" style="width: 100px;">Contas pagas?</label>                
                </div>
                <div class="form-group">
                    <select id="contas-pagas" class="form-control" name="contas_pagas">
                        <option value="0" <?php echo $contas_pagas_ == 0 ? "selected" : ""; ?> >Não</option>
                        <option value="1" <?php echo $contas_pagas_ == 1 ? "selected" : ""; ?> >Sim</option>
                    </select>                   
                </div>
            </div>
    ...
    <hr/>
    <div>
        <table id="table-contas-pagar" class="table table-responsive table-bordered">
            <thead>
                <tr>
                    <th>Despesa</th>
...
<!-- Modal Pagar Parcela -->
<div class="modal fade" id="modalPagarParcela" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="form-pagar-parcela" class="form-horizontal" method="post" data-action="<?php echo $this->Html->url(array("controller" => "contas_pagar", "action" => "pagar_parcela")); ?>">
...
<script type="text/javascript">
    ...
    jQuery(document).ready(function () {
        ...
        $('#tipos-financeiros, #planos-contas, #favorecidos, #contas-pagas, #clinica').select2({
            autocomplete: true,
            width: "100%"
        });
    ...
    debugger;
        if (idfinanceiro !== null && idfinanceiro !== undefined) {
            jQuery.ajax({
                url: $NOME_APLICACAO + "/contas_pagar/ajax_parcela_detalhes?idfinanceiro=" + idfinanceiro,
                type: 'POST',
    ...
    function consultar() {
        if (!carregando) {
            ...
            jQuery.ajax({
                ...
                data: data,
                success: function (data, textStatus, jqXHR) {
                    jQuery("#table-contas-pagar tbody").html("");
                    jQuery("#table-contas-pagar tbody").html(data);
            ...
            $('html, body').animate({
                scrollTop: $("#table-contas-pagar").offset().top
            }, 2000);
    ...
    function mostrarDetalhes(iddespesa) {
        $("#despesa-" + iddespesa).toggle('slow');
    }

    function pagarParcela() {
        var elementform = jQuery("#form-pagar-parcela");
        var formURL = jQuery("#form-pagar-parcela").attr("data-action");
        //plugin form.min
