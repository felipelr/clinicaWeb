...
class User extends AppModel
{
    ...
    public function validaLogin($email, $senha)
    {
        ...
        $db = $this->getDataSource();
        $dados = $db->fetchRow("SELECT * FROM usuario WHERE ativo = 1 AND email = '$email' and senha = '$senha' limit 1;");
        return $dados["usuario"];
    }
    ...
    public function retornarPorSenha($senha)
    {
        $senhaHash = Security::hash($senha);
        $dados = $this->query("SELECT * FROM usuario WHERE senha = '$senhaHash' LIMIT 1");
        return count($dados) > 0 ? $dados[0]["usuario"] : null;
    }
}
